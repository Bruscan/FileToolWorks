...
export default function VideoToWebM() {
  ...
  const convertFiles = async () => {
    ...
    for (const videoFile of videoFiles) {
      try {
        ...
        // Set video codec to VP9 (WebM codec)
        args.push("-c:v", "libvpx-vp9");

        // Set quality (CRF value: lower = better quality)
        let crfValue: string;
        if (options.quality === "low") {
          crfValue = "40"; // Lower quality, smaller file
        } else if (options.quality === "medium") {
          crfValue = "30"; // Balanced
        } else {
          crfValue = "25"; // High quality
        }
        args.push("-crf", crfValue);

        // Set bitrate to 0 for constant quality mode
        args.push("-b:v", "0");

        // Set audio codec to Opus (WebM audio codec)
        ...
        const blob = new Blob([new Uint8Array(data as Uint8Array).buffer], { type: "video/webm" });
        const url = URL.createObjectURL(blob);
        const filename = videoFile.originalName.replace(/\.[^.]+$/, ".webm");
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          {!loadingFFmpeg && videoFiles.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-4">
                Supports: MP4, AVI, MOV, MKV, FLV, and all video formats
              </p>
            </div>
          ) : !loadingFFmpeg && videoFiles.length > 0 ? (
            <div>
                  ...
                  <div className="px-4 pb-4 pt-2 border-t border-gray-200 space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        WebM uses VP9 codec for excellent compression. High quality produces better visuals with larger files, while Low quality creates smaller files for faster web loading.
                      </p>
                    </div>
                  </div>
                )}
      ...
      {/* Related Tools */}
      <RelatedTools currentToolId="video-to-webm" />
    </div>
  );
}
